	...
	window.CKEDITOR && window.CKEDITOR.dom || (window.CKEDITOR || (window.CKEDITOR = function() {
			...
			CKEDITOR.event.prototype = function() {
				...
				b.prototype = {
					getListenerIndex: function(a) {
						for(var b = 0, m = this.listeners; b < m.length; b++)
							if(m[b].fn == a) return b;
						return -1
					}
				};
				return {
					...
					on: function(b, d, m, e, f) {
						...
						if(0 > g.getListenerIndex(d)) {
							...
							for(var u = g.length - 1; 0 <= u; u--)
								if(g[u].priority <= f) return g.splice(u + 1, 0, h), {
									removeListener: k
								};
							g.unshift(h)
						}
					...
					fire: function() {
						...
						return function(f, h, k) {
							...
							if(g) {
								var u = g.listeners;
								if(u.length)
									for(var u = u.slice(0), q, v = 0; v < u.length; v++) {
										if(g.errorProof) try {
											q = u[v].call(this, k, h, b, l)
										} catch(t) {} else q = u[v].call(this, k, h, b, l);
			...
			if(window.console && window.console.log) {
				var e = console[a.data.type] ? a.data.type : "log",
					b = a.data.errorCode;
				if(a = a.data.additionalData) console[e]("[CKEDITOR] " + b, a);
				else console[e]("[CKEDITOR] " + b);
				console[e]("[CKEDITOR] For more information go to http://docs.ckeditor.com/#!/guide/dev_errors-section-" + b)
			}
		}, null, null, 999), CKEDITOR.dom = {}, function() {
			var a = [],
				e = CKEDITOR.env.gecko ? "-moz-" : CKEDITOR.env.webkit ? "-webkit-" : CKEDITOR.env.ie ?
				"-ms-" : "",
				b = /&/g,
				c = />/g,
				d = /</g,
			...
			CKEDITOR.tools = {
				...
				buildStyleHtml: function(a) {
					...
					for(var g, b = [], f = 0; f < a.length; f++)
						if(g = a[f]) /@import|[{}]/.test(g) ? b.push("\x3cstyle\x3e" +
				...
				normalizeCssText: function(a, g) {
					var b = [],
						f, h = CKEDITOR.tools.parseCssText(a, !0, g);
				...
				checkIfAnyArrayItemMatches: function(a, g) {
					for(var b = 0, f = a.length; b < f; ++b)
						if(a[b].match(g)) return !0;
					return !1
				},
				checkIfAnyObjectPropertyMatches: function(a, g) {
					for(var b in a)
						if(b.match(g)) return !0;
					return !1
				},
		...
		function(a) {
			...
			getIndex: function(a) {
				...
				do
					if(!a || b == this.$ || b.nodeType != CKEDITOR.NODE_TEXT || !d && b.nodeValue) c++, d = b.nodeType == CKEDITOR.NODE_TEXT; while (b = b.previousSibling);
				return c
			},
			...
			CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
				...
				appendBogus: function(a) {
					if(a || CKEDITOR.env.needsBrFiller) {
						for(a = this.getLast(); a &&
							a.type == CKEDITOR.NODE_TEXT && !CKEDITOR.tools.rtrim(a.getText());) a = a.getPrevious();
				...
				moveChildren: function(a, b) {
					...
					if(k != a) {
						var g;
						if(b)
							for(; g = k.lastChild;) a.insertBefore(k.removeChild(g), a.firstChild);
						else
							for(; g = k.firstChild;) a.appendChild(k.removeChild(g))
					}
				},
				...
				getDocumentPosition: function(a) {
					...
					if(document.documentElement.getBoundingClientRect) {
						var m = this.$.getBoundingClientRect(),
							l = g.$.documentElement,
							t = l.clientTop || d.$.clientTop ||
							0,
							p = l.clientLeft || d.$.clientLeft || 0,
							w = !0;
						CKEDITOR.env.ie && (w = g.getDocumentElement().contains(this), g = g.getBody().contains(this), w = e && g || !e && w);
						w && (CKEDITOR.env.webkit || CKEDITOR.env.ie && 12 <= CKEDITOR.env.version ? (b = d.$.scrollLeft || l.scrollLeft, c = d.$.scrollTop || l.scrollTop) : (c = e ? d.$ : l, b = c.scrollLeft, c = c.scrollTop), b = m.left + b - p, c = m.top + c - t)
					} else
						for(t = this, p = null; t && "body" != t.getName() && "html" != t.getName();) {
							b += t.$.offsetLeft - t.$.scrollLeft;
							c += t.$.offsetTop - t.$.scrollTop;
							t.equals(this) || (b += t.$.clientLeft ||
								0, c += t.$.clientTop || 0);
							for(; p && !p.equals(t);) b -= p.$.scrollLeft, c -= p.$.scrollTop, p = p.getParent();
							p = t;
							t = (m = t.$.offsetParent) ? new CKEDITOR.dom.element(m) : null
						}
					a && (m = this.getWindow(), t = a.getWindow(), !m.equals(t) && m.$.frameElement && (a = (new CKEDITOR.dom.element(m.$.frameElement)).getDocumentPosition(a), b += a.x, c += a.y));
				...
				copyAttributes: function(a,
					...
					for(var g = 0; g < c.length; g++) {
						var d = c[g],
							...
							m;
						if(!(e in b))
							if("checked" == e && (m = this.getAttribute(e))) a.setAttribute(e, m);
							else if(!CKEDITOR.env.ie || this.hasAttribute(e)) m = this.getAttribute(e), null === m && (m = d.nodeValue), a.setAttribute(e, m)
					}
		...
		function() {
			...
			function e(a, b, c, d, f) {
				function k(a, b, g, c) {
					...
					if(c && m) return d;
					r || c ? b.append(a.clone(!0, f), g) : (a.remove(), l && b.append(a));
					return d
				}
				...
				function h() {
					...
					for(a = 0; a < c; a++)
						if(b = D[a], g = N[a], !b.equals(g)) return a;
					return a - 1
				}
				...
				var z = a.startContainer,
					...
					A = a.endOffset,
					C, G, F, I, H, K;
				if(r && E.type == CKEDITOR.NODE_TEXT && z.equals(E)) z = a.document.createText(z.substring(y, A)), c.append(z);
				else {
					E.type == CKEDITOR.NODE_TEXT ? r ? K = !0 : E = E.split(A) : 0 < E.getChildCount() ? A >= E.getChildCount() ? (E = E.getChild(A - 1), G = !0) : E = E.getChild(A) : I = G = !0;
					z.type == CKEDITOR.NODE_TEXT ? r ? H = !0 : z.split(y) : 0 < z.getChildCount() ? 0 === y ? (z = z.getChild(y), C = !0) : z = z.getChild(y - 1) : F = C = !0;
					...
					O = c;
					for(J = R; J <= T; J++)
						if(c = N[J], W = c.getPrevious(), c.equals(D[J])) b && (O = O.getChild(0));
						else {
			...
			CKEDITOR.dom.range.prototype = {
				...
				createBookmark2: function() {
					function a(g) {
						...
						if(c.type == CKEDITOR.NODE_TEXT) {
							f = c;
							for(k = 0;
								(f = f.getPrevious()) && f.type == CKEDITOR.NODE_TEXT;) k += f.getLength();
							f = k;
				...
				enlarge: function(a, b) {
					...
					switch(a) {
						...
						case CKEDITOR.ENLARGE_ELEMENT:
							...
							A.type == CKEDITOR.NODE_TEXT ? (G && (A = !CKEDITOR.tools.trim(A.substring(0, G)).length && A, y = !!A), A && ((z = A.getPrevious()) || (r = A.getParent()))) : (G && (z = A.getChild(G - 1) || A.getLast()),
								z || (r = A));
							for(r = c(r); r || z;) {
								if(r && !z) {
									!E && r.equals(h) && (E = !0);
									if(f ? r.isBlockBoundary() : !e.contains(r)) break;
									y && "inline" == r.getComputedStyle("display") || (y = !1, E ? m = r : this.setStartBefore(r));
									z = r.getPrevious()
								}
								for(; z;)
									if(A = !1, z.type == CKEDITOR.NODE_COMMENT) z = z.getPrevious();
									else {
										if(z.type == CKEDITOR.NODE_TEXT) C = z.getText(), d.test(C) && (z = null), A = /[\s\ufeff]$/.test(C);
										else if((z.$.offsetWidth > (CKEDITOR.env.webkit ? 1 : 0) || b && z.is("br")) && !z.data("cke-bookmark"))
											if(y && CKEDITOR.dtd.$removeEmpty[z.getName()]) {
												C =
													z.getText();
												if(d.test(C)) z = null;
												else
													for(var G = z.$.getElementsByTagName("*"), F = 0, I; I = G[F++];)
										...
										A && (y ? E ? m = r : r && this.setStartBefore(r) : y = !0);
										if(z) {
											A = z.getPrevious();
											if(!r && !A) {
												r = z;
							...
							A.type == CKEDITOR.NODE_TEXT ? CKEDITOR.tools.trim(A.substring(G)).length ? y = !0 : (y = !A.getLength(), G == A.getLength() ? (z = A.getNext()) ||
								(r = A.getParent()) : k(A, G) && (r = A.getParent())) : (z = A.getChild(G)) || (r = A);
							for(; r || z;) {
								if(r && !z) {
									!E && r.equals(h) && (E = !0);
									if(f ? r.isBlockBoundary() : !e.contains(r)) break;
									y && "inline" == r.getComputedStyle("display") || (y = !1, E ? l = r : r && this.setEndAfter(r));
									z = r.getNext()
								}
								for(; z;) {
									A = !1;
									if(z.type == CKEDITOR.NODE_TEXT) C = z.getText(), k(z, 0) || (z = null), A = /^[\s\ufeff]/.test(C);
										...
										if((0 < z.$.offsetWidth || b && z.is("br")) && !z.data("cke-bookmark"))
											if(y && CKEDITOR.dtd.$removeEmpty[z.getName()]) {
												...
												if(d.test(C)) z = null;
													...
													for(G = z.$.getElementsByTagName("*"), F = 0; I = G[F++];)
														if(!CKEDITOR.dtd.$removeEmpty[I.nodeName.toLowerCase()]) {
															z = null;
															break
														}
						...
						case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
							...
							var H, K, D = CKEDITOR.dom.walker.blockBoundary(a == CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS ? {
									br: 1
								} : null),
								N = null,
								R = function(a) {
							...
							r = r.lastBackward();
							H = H || e;
							this.setStartAt(H, !H.is("br") && (!r && this.checkStartOfBlock() || r && H.contains(r)) ? CKEDITOR.POSITION_AFTER_START : CKEDITOR.POSITION_AFTER_END);
							...
							r = r.lastForward();
							H = H || e;
							this.setEndAt(H, !r && this.checkEndOfBlock() || r && H.contains(r) ? CKEDITOR.POSITION_BEFORE_END : CKEDITOR.POSITION_BEFORE_START);
				...
				moveToClosestEditablePosition: function(a, b) {
					var c, d = 0,
						f, k, h = [CKEDITOR.POSITION_AFTER_END, CKEDITOR.POSITION_BEFORE_START];
					a ? (c = new CKEDITOR.dom.range(this.root), c.moveToPosition(a, h[b ? 0 : 1])) : c = this.clone();
					if(a && !a.is(CKEDITOR.dtd.$block)) d = 1;
					else if(f = c[b ? "getNextEditableNode" : "getPreviousEditableNode"]()) d = 1, (k = f.type ==
			...
			a.prototype = {
				getNextParagraph: function(a) {
					...
					for(this._.nextNode = null; p;) {
						var w = 0,
							x = p.hasAscendant("pre"),
							B = p.type != CKEDITOR.NODE_ELEMENT,
							r = 0;
						if(B) p.type == CKEDITOR.NODE_TEXT && d.test(p.getText()) && (B = 0);
						else {
							var z = p.getName();
							if(CKEDITOR.dtd.$block[z] && "false" == p.getAttribute("contenteditable")) {
								...
								b(this, a, g);
								break
							} else if(p.isBlockBoundary(this.forceBrBreak && !x && {
									br: 1
								})) {
								if("br" == z) B = 1;
								else if(!e && !p.getChildCount() && "hr" != z) {
									g = p;
									l = p.equals(t);
									break
								}
						...
						if(e && !w)
							for(; !p.getNext(f) && !l;) {
								z = p.getParent();
								if(z.isBlockBoundary(this.forceBrBreak && !x && {
										br: 1
									})) {
									w = 1;
									B = 0;
					...
					this._.nextNode || (this._.nextNode = l || g.equals(t) || !t ? null : this._getNextSourceNode(g, 1, t));
					return g
				},
		...
		function() {
			function a(a, b, c, d, g) {
				...
				for(f in b) {
					k = b[f];
					k = "boolean" == typeof k ? {} : "function" == typeof k ? {
						match: k
					} : F(k);
					...
					for(m in K) {
						...
						var r = e,
							...
							n = void 0;
						t ? E = !1 : t = {};
						for(n in B) "!" == n.charAt(0) && (n = n.slice(1), p.push(n), t[n] = !0, E = !1);
				...
				for(d = a.length; c < d; ++c) {
					...
					for(m in D) {
						...
						t = [];
						B = void 0;
						for(B in z) - 1 < B.indexOf("*") ? t.push(new RegExp("^" + B.replace(/\*/g, ".*") + "$")) : t.push(B);
						z = t;
						z.length &&
							(e[h] = z, r = !1)
					}
					...
					if(!0 === f.elements || null === f.elements) g[k ? "unshift" : "push"](f);
					else
						for(v in e = f.elements, delete f.elements, e)
							if(b[v]) b[v][k ? "unshift" : "push"](f);
							else b[v] = [f]
				}
			}
			...
			function c(a, b, c) {
				if(!a.match || a.match(b)) {
					...
					b = b.classes;
					if(a) {
						for(var f = !1, g = !0 === a, k = b.length; k--;)
							if(g ||
								a(b[k])) b.splice(k, 1), f = !0;
						a = f
					} else a = !1;
			...
			function q(a, b, d, f) {
				...
				if(f.doCallbacks && a.elementCallbacks) {
					a: {
						...
						for(var h = 0, m = k.length, r; h < m; ++h)
							if(r = k[h](b)) {
								k = r;
								break a
							}
				...
				if(f.doFilter) {
					a: {
						h = b.name;m = a._;a = m.allowedRules.elements[h];k = m.allowedRules.generic;h =
						m.disallowedRules.elements[h];m = m.disallowedRules.generic;r = f.skipRequired;
						var l = {
								valid: !1,
								validAttributes: {},
								validClasses: {},
						...
						if(a || k) {
							...
							if(m)
								for(D = 0, B = m.length; D < B; ++D) c(m[D], b, l);
							if(a)
								for(D = 0, B = a.length; D < B; ++D) e(a[D], b, l, r);
							if(k)
								for(D = 0, B = k.length; D < B; ++D) e(k[D], b, l, r);
							a = l
						} else a = null
					}
					if(!a || !a.valid) return d.push(b),
						1;B = a.validAttributes;
					var w = a.validStyles;k = a.validClasses;
					var h = b.attributes,
						n = b.styles,
						m = b.classes;r = b.classBackup;
					var E = b.styleBackup,
						v, y, A = [],
						l = [],
						x = /^data-cke-/;D = !1;delete h.style;delete h["class"];delete b.classBackup;delete b.styleBackup;
					if(!a.allAttributes)
						for(v in h) B[v] || (x.test(v) ? v == (y = v.replace(/^data-cke-saved-/, "")) || B[y] || (delete h[v], D = !0) : (delete h[v], D = !0));
					if(!a.allStyles || a.hadInvalidStyle) {
						for(v in n) a.allStyles || w[v] ? A.push(v + ":" + n[v]) : D = !0;
					...
					if(!a.allClasses || a.hadInvalidClass) {
						...
						r && l.length < r.split(/\s+/).length && (D = !0)
					} else r && (h["class"] = r);D && (g = 1);
			...
			function B(a) {
				return a.type ==
					CKEDITOR.NODE_ELEMENT && ("br" == a.name || G.$block[a.name])
			}
			...
			function r(a, b, c) {
				...
				if(G.$empty[d] || !a.children.length) "hr" == d && "br" == b ? a.replaceWith(x()) : (a.parent && c.push({
					...
					if("br" == b) a.previous && !B(a.previous) && (b = x(), b.insertBefore(a)), a.next && !B(a.next) && (b = x(), b.insertAfter(a)), a.replaceWithChildren();
					else {
						var d = a.children,
							f;
						b: {
						...
						if(f) a.name = b, a.attributes = {}, c.push({
							...
							for(var g = f.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || "body" == f.name, h, m, k = d.length; 0 < k;) e = d[--k], g && (e.type == CKEDITOR.NODE_TEXT || e.type == CKEDITOR.NODE_ELEMENT && G.$inline[e.name]) ? (h || (h = new CKEDITOR.htmlParser.element(b), h.insertAfter(a), c.push({
								check: "parent-down",
								el: h
							})), h.add(e, 0)) : (h = null, m = G[f.name] || G.span, e.insertAfter(a), f.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || e.type != CKEDITOR.NODE_ELEMENT || m[e.name] ||
								c.push({
									check: "el-up",
									el: e
								}));
			...
			function E(a, b) {
				...
				for(g in d)
					if("class" == g)
						for(c = d[g].split(/\s+/), e = a.classes.join("|"); h = c.pop();) {
							if(-1 == e.indexOf(h)) return !1
						} else if(a.attributes[g] !=
							d[g]) return !1;
				for(k in f)
					if(a.styles[k] != f[k]) return !1;
				return !0
			}
			...
			CKEDITOR.filter.prototype = {
				...
				applyTo: function(a, b, c, d) {
							...
							a: {
								...
								if(k)
									for(l = 0; l < k.length; ++l)
										if((D = d.match(k[l])) && D[0].length == d.length) {
											c = !0;
											break a
										}
								d = CKEDITOR.htmlParser.fragment.fromHtml(d);1 == d.children.length && (r = d.children[0]).type == CKEDITOR.NODE_ELEMENT && q(f, r, c, m);c = !c.length
							}
							c || g.push(a)
						}
					}, null, !0);
					...
					for(var D; c = g.pop();) c.type == CKEDITOR.NODE_ELEMENT ? r(c, d, a) : c.remove();
					for(; l = a.pop();)
						if(c = l.el, c.parent) switch(D = G[c.parent.name] || G.span, l.check) {
							case "it":
								G.$removeEmpty[c.name] && !c.children.length ? r(c, d, a) : p(c) || r(c, d, a);
								break;
							case "el-up":
								c.parent.type ==
									CKEDITOR.NODE_DOCUMENT_FRAGMENT || D[c.name] || r(c, d, a);
								break;
							case "parent-down":
								c.parent.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || D[c.name] || r(c.parent, d, a)
						}
					return h
				},
					...
					if(!this.disabled && a) {
						...
						for(f = 0; f < a.length; ++f) {
							...
							var g = void 0,
								...
								r = void 0;
							c = [];
							for(k = 0; k < b.length; ++k) e = b[k], "string" == typeof e ? (e = e.split(/\s*:\s*/), h = e[0], m = null, r = e[1]) : (h = e.check, m = e.left, r = e.right), g || (g = e, g = g.element ? g.element : h ? h.match(/^([a-z0-9]+)/i)[0] :
								g.left.getDefinition().element), m instanceof CKEDITOR.style && (m = A(m)), c.push({
								check: h == g ? null : h,
								left: m,
								right: "string" == typeof r ? C(r) : r
							});
							b = g;
							d[b] || (d[b] = []);
							d[b].push(c)
				...
				O = CKEDITOR.filter.transformationsTools = {
					...
					transform: function(a, b) {
						if("string" == typeof b) a.name = b;
							...
							for(g in f)
								if("class" == g)
									for(c = a.classes.join("|"), e = f[g].split(/\s+/); h = e.pop();) - 1 == c.indexOf(h) && a.classes.push(h);
					...
					l = function(b) {
						a.call(this, b, function(a) {
							...
							for(f in a) {
								...
								if(!e[f]) {
									if(n.icons)
										for(var q = n.icons.split(","), v = q.length; v--;) CKEDITOR.skin.addIcon(q[v], n.path + "icons/" + (CKEDITOR.env.hidpi && n.hidpi ? "hidpi/" : "") + q[v] + ".png");
									e[f] = 1
								}
								if(u)
									for(u.split && (u = u.split(",")), n = 0; n < u.length; n++) m[u[n]] ||
										b.push(u[n])
							}
		...
		function() {
			...
			function n(a) {
				...
				if(d) var g = new RegExp("(?:^|,)(?:" + d.replace(/\s*,\s*/g, "|") + ")(?\x3d,|$)", "g"),
					c = c.replace(g, ""),
					c = c + ("," + d);
				if(f) var k = new RegExp("(?:^|,)(?:" + f.replace(/\s*,\s*/g, "|") + ")(?\x3d,|$)", "g"),
					c = c.replace(k, "");
				...
				CKEDITOR.plugins.load(c.split(","), function(c) {
					...
					for(var e in c) {
						...
						if(l && (D = l.match(k)))
							for(; l = D.pop();) CKEDITOR.error("editor-plugin-required", {
								plugin: l.replace(",", ""),
								requiredBy: e
							});
						m && !a.lang[e] && (m.split && (m = m.split(",")), 0 <= CKEDITOR.tools.indexOf(m, a.langCode) ? r = a.langCode : (r = a.langCode.replace(/-.*/, ""), r = r != a.langCode && 0 <= CKEDITOR.tools.indexOf(m, r) ? r : 0 <= CKEDITOR.tools.indexOf(m, "en") ? "en" : m[0]), h.langEntries && h.langEntries[r] ? (a.lang[e] = h.langEntries[r], r = null) : g.push(CKEDITOR.getUrl(h.path + "lang/" + r + ".js")));
						f.push(r);
						d.push(h)
					}
			...
			CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
				...
				execCommand: function(a, b) {
					...
					return c && c.state != CKEDITOR.TRISTATE_DISABLED && !1 !== this.fire("beforeCommandExec",
						d) && (d.returnValue = c.exec(d.commandData), !c.async && !1 !== this.fire("afterCommandExec", d)) ? d.returnValue : !1
				},
			...
			CKEDITOR.htmlParser.prototype = {
				...
				parse: function(b) {
					for(var c, d, m = 0, l; c = this._.htmlPartsRegex.exec(b);) {
						...
						if(d = c[1])
							if(d = d.toLowerCase(), l && CKEDITOR.dtd.$cdata[d] && (this.onCDATA(l.join("")), l = null), !l) {
								this.onTagClose(d);
								continue
							}
						if(l) l.push(c[0]);
			...
			CKEDITOR.htmlParser.fragment.fromHtml = function(m, l, f) {
				...
				function n(b, c, d) {
					...
					void 0 === b.previous && (u(c, b) && (x = c, v.onTagOpen(f, {}), b.returnPoint = c = x), g(b), a(b) && !b.children.length || c.add(b), "pre" ==
						b.name && (r = !1), "textarea" == b.name && (B = !1));
				...
				var v =
					...
					t = l instanceof CKEDITOR.htmlParser.element ? l : "string" == typeof l ? new CKEDITOR.htmlParser.element(l) : new CKEDITOR.htmlParser.fragment,
					p = [],
					w = [],
					x = t,
					B = "textarea" == t.name,
				...
				v.onTagOpen = function(d, f, g, m) {
					...
					if(a(f)) p.push(f);
						...
						if("br" == d) w.push(f);
							...
							for(; !(m = (g = x.name) ?
									CKEDITOR.dtd[g] || (x._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span) : c, f.isUnknown || x.isUnknown || m[d]);)
								if(x.isOptionalClose) v.onTagClose(g);
								else if(d in b && g in b) g = x.children, (g = g[g.length - 1]) && "li" == g.name || n(g = new CKEDITOR.htmlParser.element("li"), x), !f.returnPoint && (f.returnPoint = x), x = g;
							else if(d in CKEDITOR.dtd.$listItem && !q(d, g)) v.onTagOpen("li" == d ? "ul" : "dl", {}, 0, 1);
							else if(g in e && !q(d, g)) !f.returnPoint && (f.returnPoint = x), x = x.parent;
							else if(g in CKEDITOR.dtd.$inline && p.unshift(x), x.parent) n(x,
								x.parent, 1);
							else {
								f.isOrphan = 1;
								break
				...
				v.onTagClose = function(a) {
					...
					for(var c = [], d = [], g = x; g != t && g.name != a;) g._.isBlockLike || d.unshift(g), c.push(g), g = g.returnPoint || g.parent;
					if(g != t) {
				...
				v.onText = function(a) {
					if(!(x._.hasInlineStarted && !w.length || r || B) && (a = CKEDITOR.tools.ltrim(a),
							0 === a.length)) return;
					var b = x.name,
						g = b ? CKEDITOR.dtd[b] || (x._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span) : c;
					if(!B && !g["#"] && b in e) v.onTagOpen(d[b] || ""), v.onText(a);
					else {
						k();
						h();
						r || B || (a = a.replace(/[\t\r\n ]{2,}|[\t\r\n]/g, " "));
			...
			a.prototype = {
				...
				exec: function(a, c) {
					var d = c instanceof CKEDITOR.htmlParser.node || c instanceof CKEDITOR.htmlParser.fragment,
						...
						h, k, g, n;
					for(n = 0; n < f; n++)
						if(d && (h = c.type, k = c.name), g = l[n], !(a.nonEditable && !g.options.applyToAll || a.nestedEditable && g.options.excludeNestedEditable)) {
		...
		function() {
			function a(a, f) {
					...
					return function(f) {
						if(f.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
							var e = [],
								...
								r, l;
							if(h)
								for(k(h, 1) && e.push(h); h;) m(h) &&
									(r = c(h)) && k(r) && ((l = c(r)) && !m(l) ? e.push(r) : (g(D).insertAfter(r), r.remove())), h = h.previous;
							for(h = 0; h < e.length; h++) e[h].remove();
							if(e = !a || !1 !== ("function" == typeof d ? d(f) : d)) D || CKEDITOR.env.needsBrFiller || f.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT ? D || CKEDITOR.env.needsBrFiller || !(7 < document.documentMode || f.name in CKEDITOR.dtd.tr || f.name in CKEDITOR.dtd.$listItem) ? (e = b(f), e = !e || "form" == f.name && "input" == e.name) : e = !1 : e = !1;
							e && f.add(g(a))
						}
					}
				}
				...
				var h = {
					...
					D = "html" == f,
					B = CKEDITOR.tools.extend({}, r),
					z;
				for(z in B) "#" in x[z] || delete B[z];
				for(z in B) h.elements[z] = e(D, a.config.fillEmptyBlocks);
				h.root = e(D, !1);
				h.elements.br =
					function(a) {
						return function(b) {
							if(b.parent.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
								...
								if("data-cke-bogus" in f || "data-cke-eol" in f) delete f["data-cke-bogus"];
									...
									var e = c(b);
									!f && m(b.parent) ? l(b.parent, g(a)) : m(f) && e && !m(e) && g(a).insertBefore(f)
								}
							}
						}
					}(D);
			...
			var p = /(?:&nbsp;|\xa0)$/,
				...
				r = CKEDITOR.tools.extend({}, x.$blockLimit, x.$block),
				z = {
					elements: {
						input: f,
						textarea: f
		...
		function() {
			...
			CKEDITOR.inlineAll = function() {
				...
				for(b in CKEDITOR.dtd.$editable)
					for(var c = CKEDITOR.document.getElementsByTag(b), d = 0, m = c.count(); d < m; d++) a = c.getItem(d), "true" == a.getAttribute("contenteditable") && (e = {
						element: a,
						config: {}
		...
		function() {
			...
			function b(a) {
				var b = a.name,
					...
					k = new CKEDITOR.template('\x3c{outerEl} id\x3d"cke_{name}" class\x3d"{id} cke cke_reset cke_chrome cke_editor_{name} cke_{langDir} ' +
			...
			CKEDITOR.editor.prototype.setMode = function(a, b) {
				...
				if(a != e.mode && l && l[a]) {
					...
					if(e.mode) {
						...
						"source" == e.mode && l == h && (e.fire("lockSnapshot", {
							forceUpdate: !0
						}), k = 1);
				...
				proto: {
					focus: function() {
						...
						if(CKEDITOR.env.webkit &&
							!this.hasFocus && (a = this.editor._.previousActive || this.getDocument().getActive(), this.contains(a))) {
							a.focus();
							return
						}
						try {
					...
					insertElementIntoRange: function(a, b) {
						...
						if(g)
							for(;
								(e = b.getCommonAncestor(0, 1)) && (k = CKEDITOR.dtd[e.getName()]) && (!k || !k[f]);) e.getName() in CKEDITOR.dtd.span ? b.splitElement(e) :
									...
									if(c in h) {
										...
										CKEDITOR.env.ie && 11 > CKEDITOR.env.version && (f = b.getSelectedElement()) || (f = m(b)) ? (a.fire("saveSnapshot"), e.moveToPosition(f, CKEDITOR.POSITION_BEFORE_START), f.remove(), e.select(), a.fire("saveSnapshot"), d = 1) : e.collapsed && ((l = k.block) && (r = l[c ? "getPrevious" : "getNext"](g)) && r.type == CKEDITOR.NODE_ELEMENT && r.is("table") && e[c ? "checkStartOfBlock" : "checkEndOfBlock"]() ?
											(a.fire("saveSnapshot"), e[c ? "checkEndOfBlock" : "checkStartOfBlock"]() && l.remove(), e["moveToElementEdit" + (c ? "End" : "Start")](r), e.select(), a.fire("saveSnapshot"), d = 1) : k.blockLimit && k.blockLimit.is("td") && (D = k.blockLimit.getAscendant("table")) && e.checkBoundaryOfElement(D, c ? CKEDITOR.START : CKEDITOR.END) && (r = D[c ? "getPrevious" : "getNext"](g)) ? (a.fire("saveSnapshot"), e["moveToElementEdit" + (c ? "End" : "Start")](r), e.checkStartOfBlock() && e.checkEndOfBlock() ? r.remove() : e.select(), a.fire("saveSnapshot"), d = 1) : (D = k.contains(["td",
												"th", "caption"
											])) && e.checkBoundaryOfElement(D, c ? CKEDITOR.START : CKEDITOR.END) && (d = 1))
									}
									return !d
								});
			...
			var g = CKEDITOR.dom.walker.whitespaces(!0),
				...
				t = function() {
					...
					function b(c, d) {
						...
						f = d.range.startPath();
						for(var l = m[l.getName()], n = 0, w = c.getChildren(), v = w.count(), p = -1, z = -1, t = 0, E = f.contains(m.$list); n < v; ++n) f = w.getItem(n), a(f) ? (e =
							f.getName(), E && e in CKEDITOR.dtd.$list ? h = h.concat(b(f, d)) : (k = !!l[e], "br" != e || !f.data("cke-eol") || n && n != v - 1 || (t = (g = n ? h[n - 1].node : w.getItem(n + 1)) && (!a(g) || !g.is("br")), g = g && a(g) && m.$block[g.getName()]), -1 != p || k || (p = n), k || (z = n), h.push({
								isElement: 1,
								isLineBreak: t,
							...
							node: f,
							allowed: 1
						}); - 1 < p && (h[p].firstNotAllowed = 1); - 1 < z && (h[z].lastNotAllowed = 1);
						return h
					}
					...
					function d(b, c) {
						var f = [],
							...
							k, h = 0,
							l = m[c],
							r = !b.is(m.$inline) || b.is("br");
						for(r && f.push(" "); h < e; h++) k = g.getItem(h), a(k) && !k.is(l) ? f = f.concat(d(k, c)) : f.push(k);
						r && f.push(" ");
						return f
					}
					...
					delete v.br;
					return function(n, D, p, t) {
						var q = n.editor,
							x = !1;
						"unfiltered_html" == D && (D = "html", x = !0);
						if(!t.checkReadOnly()) {
							var u = (new CKEDITOR.dom.elementPath(t.startContainer, t.root)).blockLimit || t.root;
							n = {
								type: D,
								dontFilter: x,
								editable: n,
							...
							if(!D.collapsed) {
								var q = u.block || u.blockLimit,
									W = D.getCommonAncestor();
								q && !q.equals(W) && !q.contains(W) && D.checkEndOfBlock() && n.zombies.push(q);
								D.deleteContents()
							}
							for(;
								(M = g(D)) && a(M) && M.isBlockBoundary() && x.contains(M);) D.moveToPosition(M, CKEDITOR.POSITION_BEFORE_END);
							...
							F = D.createBookmark();
							(q = F.startNode.getPrevious(c)) && a(q) && e(q) && t.push(q);
							(q = F.startNode.getNext(c)) && a(q) && e(q) && t.push(q);
							for(q = F.startNode;
								(q = q.getParent()) && e(q);) t.push(q);
							D.moveToBookmark(F);
							if(F = p) {
								F = n.range;
								if("text" == n.type && n.inlineStylesRoot) {
									...
									for(t = n.inlineStylesRoot.getParent(); !M.equals(t);) D = D.appendTo(M.clone()), M = M.getParent();
								...
								if((U = F.startPath().block) && (1 !=
									...
									if(1 == M.getChildCount() && a(J = M.getFirst()) && J.is(w) && !J.hasAttribute("contenteditable")) {
										...
										F = 0;
										for(t = U.count(); F < t; F++)
											if(D = U.getItem(F), !D.is(v)) break a;
							...
							if(F) {
								...
								var x = n.endPath.elements[0],
									ba, u = x.getPosition(D),
									W = !!x.getCommonAncestor(D) && u != CKEDITOR.POSITION_IDENTICAL && !(u &
										CKEDITOR.POSITION_CONTAINS + CKEDITOR.POSITION_IS_CONTAINED);
								D = b(n.dataWrapper, n);
								for(h(D, J); t < D.length; t++) {
									u = D[t];
									if(L = u.isLineBreak) {
										L = J;
										V = M;
									...
									if(L) p = 0 < t;
										...
										if(q && u.isElement) {
											...
											if(L) V && (da = J.splitElement(V), n.zombies.push(da), n.zombies.push(V));
											else {
												V = M.getName();
												ba = !t;
												L = t == D.length - 1;
										...
										u.lastNotAllowed && t < D.length - 1 && ((da = W ? x : da) && J.setEndAt(da, CKEDITOR.POSITION_AFTER_START), q = 0);
										J.collapse()
									}
								}
					...
					eol: {
						detect: function(a, b) {
							var c = a.range,
								...
								e = new CKEDITOR.dom.elementPath(c.endContainer,
									b);
							d.collapse(1);
							f.collapse();
						...
						table: function() {
							function a(c) {
								...
								e.guard = function(a,
									...
									if(e && k && a.equals(k)) g = c.clone(), g.setEndAt(k, CKEDITOR.POSITION_BEFORE_END), f.push(g);
									else if(!e && h && a.equals(h)) g = c.clone(), g.setStartAt(h, CKEDITOR.POSITION_AFTER_START), f.push(g);
									else {
		...
		function() {
			...
			function c(a) {
				return t(a) || a.type == CKEDITOR.NODE_ELEMENT &&
					!a.is(CKEDITOR.dtd.$empty) ? !0 : !1
			}
				...
				if(c) {
					if(!1 !== b) {
						var d, g = a.getDocument().getSelection().getNative(),
							e = g && "None" != g.type && g.getRangeAt(0);
			...
			var q, v, t = CKEDITOR.dom.walker.invisible(1),
				p = function() {
					function a(b) {
						return function(a) {
							var c =
			...
			CKEDITOR.on("instanceCreated", function(b) {
				...
				d.on("contentDom", function() {
					...
					if(CKEDITOR.env.ie || r) w ? k.attachListener(k, "beforedeactivate", b, null, null, -1) : k.attachListener(d, "selectionCheck", b, null, null, -1), k.attachListener(k, CKEDITOR.env.webkit ? "DOMFocusOut" : "blur", function() {
						d.lockSelection(p);
						D = 1
				...
				getRanges: function() {
					var a = w ? function() {
						...
						var b = function(b, c) {
							...
							for(var f = d.children, g, e, k = b.duplicate(), h = 0, m = f.length - 1, l = -1, n, p; h <= m;)
								if(l = Math.floor((h + m) / 2), g = f[l], k.moveToElementText(g), n = k.compareEndPoints("StartToStart", b), 0 < n) m = l - 1;
								else if(0 > n) h = l + 1;
							else return {
								container: d,
								offset: a(g)
				...
				selectRanges: function(a) {
					...
					if(a.length)
						if(this.isLocked) {
							...
							a: {
								var k, h;
								if(1 == a.length && !(h = a[0]).collapsed && (e = h.getEnclosedNode()) && e.type == CKEDITOR.NODE_ELEMENT && (h = h.clone(), h.shrink(CKEDITOR.SHRINK_ELEMENT, !0), (k = h.getEnclosedNode()) && k.type == CKEDITOR.NODE_ELEMENT && (e = k), "false" == e.getAttribute("contenteditable"))) break a;e = void 0
							}
							if(e) this.fake(e);
							else {
								if(w) {
									...
									for(p = 0; p < a.length; p++) {
										...
										try {
											...
											if(0 <= v.toString().indexOf("NS_ERROR_ILLEGAL_VALUE")) e.collapse(1), n.setEnd(e.endContainer.$, e.endOffset);
											else throw v;
										}
										g.addRange(n)
									}
		...
		function() {
			...
			function b(c) {
				var f = c.document;
				if(c.collapsed) f = w(this, f), c.insertNode(f), c.moveToPosition(f, CKEDITOR.POSITION_BEFORE_END);
				else {
					var g = this.element,
						k = this._.definition,
						h, m = k.ignoreReadonly,
						l = m || k.includeReadonly;
					...
					for(x.getPosition(q) == CKEDITOR.POSITION_FOLLOWING && (x = 0); x;) {
						m = !1;
						if(x.equals(q)) x = null, m = !0;
						else {
							var B = x.type == CKEDITOR.NODE_ELEMENT ? x.getName() : null,
								u = B && "false" == x.getAttribute("contentEditable"),
								z = B && x.getAttribute("data-nostyle");
							if(B && x.data("cke-bookmark")) {
								x = x.getNextSourceNode(!0);
								continue
							}
							if(u && l && CKEDITOR.dtd.$block[B])
								for(var E = x, y = d(E), A = void 0, C = y.length, G = 0, E = C && new CKEDITOR.dom.range(E.getDocument()); G < C; ++G) {
									var A = y[G],
										ga = CKEDITOR.filter.instances[A.data("cke-filter")];
									if(ga ? ga.check(this) : 1) E.selectNodeContents(A), b.call(this, E)
								}
							y = B ? !n[B] || z ? 0 : u && !l ? 0 : e(x, q, k, H) : 1;
							if(y)
								if(A = x.getParent(), y = k, C = g, G = h, !A || !(A.getDtd() || CKEDITOR.dtd.span)[C] && !G || y.parentRule && !y.parentRule(A)) m = !0;
								else {
									if(r || B && CKEDITOR.dtd.$removeEmpty[B] && (x.getPosition(q) | H) != H || (r = c.clone(), r.setStartBefore(x)), B = x.type, B == CKEDITOR.NODE_TEXT || u || B == CKEDITOR.NODE_ELEMENT && !x.getChildCount()) {
										for(var B = x, fa;
											(m = !B.getNext(F)) && (fa = B.getParent(), n[fa.getName()]) && e(fa, t, k, K);) B = fa;
										r.setEndAfter(B)
									}
								}
							else m = !0;
							x = x.getNextSourceNode(z || u)
						}
						if(m && r && !r.collapsed) {
							for(var m = w(this, f), u = m.hasAttributes(), z = r.getCommonAncestor(), B = {}, y = {}, A = {}, C = {}, I, Z, ha; m && z;) {
								if(z.getName() == g) {
							...
							m ? (r.extractContents().appendTo(m), r.insertNode(m), v.call(this,
								m), m.mergeSiblings(), CKEDITOR.env.ie || m.$.normalize()) : (m = new CKEDITOR.dom.element("span"), r.extractContents().appendTo(m), r.insertNode(m), v.call(this, m), m.remove(!0));
							r = null
						}
					}
					c.moveToBookmark(p);
			...
			function c(a) {
				function b() {
					for(var a = new CKEDITOR.dom.elementPath(d.getParent()), c = new CKEDITOR.dom.elementPath(m.getParent()), f = null, g = null, e = 0; e < a.elements.length; e++) {
						...
						l.checkElementRemovable(k, !0) && (f = k)
					}
				...
				var c = a.createBookmark(),
					d = c.startNode;
				if(a.collapsed) {
					for(var f = new CKEDITOR.dom.elementPath(d.getParent(), a.root), g, e = 0, k; e < f.elements.length && (k = f.elements[e]) && k != f.block && k != f.blockLimit; e++)
						if(this.checkElementRemovable(k)) {
							var h;
							a.collapsed && (a.checkBoundaryOfElement(k, CKEDITOR.END) ||
								(h = a.checkBoundaryOfElement(k, CKEDITOR.START))) ? (g = k, g.match = h ? "start" : "end") : (k.mergeSiblings(), k.is(this.element) ? q.call(this, k) : t(k, r(this)[k.getName()]))
						}
					if(g) {
						k = d;
						for(e = 0;; e++) {
					...
					for(f = d; !f.equals(m);) g = f.getNextSourceNode(), f.type == CKEDITOR.NODE_ELEMENT && this.checkElementRemovable(f) && (f.getName() == this.element ? q.call(this,
						f) : t(f, r(this)[f.getName()]), g.type == CKEDITOR.NODE_ELEMENT && g.contains(d) && (b(), g = d.getNext())), f = g
				}
				a.moveToBookmark(c);
			...
			function k(a, b) {
				...
				if(d && !f) {
					...
					b = f
				} else e ? b = u(c ? [a.getHtml()] : g(a), b) : a.moveChildren(b);
				b.replace(a);
				if(d) {
					var c = b,
						h;
			...
			function q(a, b) {
				var c = this._.definition,
					...
					e;
				for(e in d)
					if("class" != e && !this._.definition.fullMatch || a.getAttribute(e) == z(e, d[e])) b &&
							...
							for(var k in c)
								if("_length" != k) {
									...
									if("style" == k) a: {
										...
										"string" == typeof f && (f = CKEDITOR.tools.parseCssText(f, !0));e = void 0;
										for(e in d)
											if(!(e in f) || f[e] != d[e] && "inherit" != d[e] && "inherit" != f[e]) {
												d = !1;
												break a
											}
										d = !0
		...
		function() {
			...
			CKEDITOR.skin = {
				...
				getIconStyle: function(a, b, c, d, f) {
					...
					d = d || e && e.offset;
					f = f || e && e.bgsize || "16px";
					return a && "background-image:url(" + CKEDITOR.getUrl(a) + ");background-position:0 " +
						d + "px;background-size:" + f + ";"
				}
			};
			CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
				getUiColor: function() {
				...
				CKEDITOR.tools.extend(CKEDITOR.ui.dialog, {
					...
					textarea: function(b, c, d) {
						if(!(3 > arguments.length)) {
							a.call(this, c);
							var e = this,
								m = this._.inputId = CKEDITOR.tools.getNextId() + "_textarea",
								l = {};
							c.validate && (this.validate = c.validate);
							l.rows = c.rows || 5;
							l.cols = c.cols || 20;
							...
							CKEDITOR.ui.dialog.labeledElement.call(this,
								...
								function() {
									for(var a = [], d = [], g = (c.id ? c.id : CKEDITOR.tools.getNextId()) + "_radio", m = 0; m < c.items.length; m++) {
										...
										f.push(new CKEDITOR.ui.dialog.uiElement(b, r, y, "input", null, E));
										y.push(" ");
		...
		function() {
			...
			function g(a) {
				function b(c) {
					var n = "rtl" == e.lang.dir,
						p = l.width,
						w = l.height,
						x = p + (c.data.$.screenX - m.x) * (n ? -1 : 1) * (a._.moved ? 1 : 2),
						v = w + (c.data.$.screenY - m.y) * (a._.moved ? 1 : 2),
						r = a._.element.getFirst(),
						r = n && r.getComputedStyle("right"),
						t = a.getPosition();
					t.y + v > h.height && (v = h.height - t.y);
					(n ? r : t.x) + x > h.width && (x = h.width - (n ? r : t.x));
			...
			CKEDITOR.dialog = function(b, f) {
				...
				function n(c) {
					if(y == CKEDITOR.dialog._.currentTop) {
						var d = c.data.getKeystroke(),
							...
							g = [37, 38, 39, 40];
						u = z = 0;
						if(9 == d || d == CKEDITOR.SHIFT + 9) l(d == CKEDITOR.SHIFT + 9 ? -1 : 1), u = 1;
						else if(d == CKEDITOR.ALT + 121 && !y._.tabBarMode && 1 < y.getPageCount()) y._.tabBarMode = !0, y._.tabs[y._.currentTabId][0].focus(), y._.currentFocusIndex = -1, u = 1;
						else if(-1 != CKEDITOR.tools.indexOf(g,
								d) && y._.tabBarMode) d = -1 != CKEDITOR.tools.indexOf([f ? 39 : 37, 38], d) ? a.call(y) : e.call(y), y.selectPage(d), y._.tabs[d][0].focus(), u = 1;
						else if(13 != d && 32 != d || !y._.tabBarMode)
							if(13 == d) d = c.data.getTarget(), d.is("a", "button", "select", "textarea") || d.is("input") && "button" == d.$.type || ((d = this.getButton("ok")) && CKEDITOR.tools.setTimeout(d.click, 0, d), u = 1), z = 1;
				...
				var x = CKEDITOR.dialog._.dialogDefinitions[f],
					v = CKEDITOR.tools.clone(p),
					t = b.config.dialog_buttonsOrder || "OS",
					q = b.lang.dir,
					B = {},
					u, z;
				("OS" == t && CKEDITOR.env.mac || "rtl" == t && "ltr" == q || "ltr" == t && "rtl" == q) && v.buttons.reverse();
				...
				if(b._.removeDialogTabs && (B = b._.removeDialogTabs[f]))
					for(q = 0; q < B.length; q++) x.removeContents(B[q]);
							...
							if(a.config.enterMode == CKEDITOR.ENTER_BR)
								for(d = !0; b.length;)
									if(k = b.shift(), "div" == k.getName()) {
										...
										for(d = k.getNext() &&
											!(k.getNext().type == CKEDITOR.NODE_ELEMENT && k.getNext().isBlockBoundary()); k.getFirst();) k.getFirst().remove().appendTo(f);
		...
		function() {
			...
			function e(b) {
				function c() {
					...
					"beforepaste" == A.mainPasteEvent && (a.on("paste", function(a) {
						...
						!a.data ||
							a.data.$.ctrlKey || a.data.$.shiftKey || k()
					}, null, null, 0));
				...
				function h(a) {
					...
					7 < CKEDITOR.env.version ?
						c.$.execCommand(a) : c.$.selection.createRange().execCommand(a);
				...
				function l(a, c) {
					...
					if(!d.getById("cke_pastebin")) {
						...
						var p = 0,
							d = d.getWindow();
						CKEDITOR.env.webkit ? (f.append(n), n.addClass("cke_editable"), f.is("body") || (p = "static" != f.getComputedStyle("position") ? f : CKEDITOR.dom.element.get(f.$.offsetParent), p = p.getDocumentPosition().y)) : f.getAscendant(CKEDITOR.env.ie ?
						...
						if(CKEDITOR.env.webkit ||
							CKEDITOR.env.gecko) k = f.once("blur", e, null, null, -100);
						p && n.focus();
				...
				b.getClipboardData = function(a, c) {
					...
					!1 === B() && (b.removeListener("paste", d), k && b.fire("pasteDialog", e) ? (b.on("pasteDialogCommit", f), b.on("dialogHide",
						function(a) {
							a.removeListener();
			...
			function c(a, b) {
				...
				if(b.match(/^[^<]$/)) return b;
				CKEDITOR.env.webkit && -1 < b.indexOf("\x3cdiv\x3e") && (b = b.replace(/^(<div>(<br>|)<\/div>)(?!$|(<div>(<br>|)<\/div>))/g, "\x3cbr\x3e").replace(/^(<div>(<br>|)<\/div>){2}(?!$)/g, "\x3cdiv\x3e\x3c/div\x3e"), b.match(/<div>(<br>|)<\/div>/) &&
					(b = "\x3cp\x3e" + b.replace(/(<div>(<br>|)<\/div>)+/g, function(a) {
			...
			function h(b) {
				...
				b.on("contentDom", function() {
					...
					h.attachListener(b, "dragstart", function() {
						...
						CKEDITOR.env.ie && 10 > CKEDITOR.env.version && (c.dragStartContainerChildCount = a ? k(a.startContainer) : null, c.dragEndContainerChildCount =
			...
			CKEDITOR.plugins.clipboard = {
				...
				fixSplitNodesAfterDrop: function(a, b, c, d) {
					function f(a, c, d) {
						...
						if(e.equals(c) && d != c.getChildCount()) return a = b.startContainer.getChild(b.startOffset - 1), c = b.startContainer.getChild(b.startOffset),
				...
				isDropRangeAffectedByDragRange: function(a, b) {
					var c = b.startContainer,
						d = b.endOffset;
					return a.endContainer.equals(c) && a.endOffset <= d || a.startContainer.getParent().equals(c) && a.startContainer.getIndex() <
				...
				getRangeAtDropPosition: function(a, b) {
					...
					if(document.caretRangeFromPoint) c =
						b.document.$.caretRangeFromPoint(d, f), h.setStart(CKEDITOR.dom.node(c.startContainer), c.startOffset), h.collapse(!0);
				...
				initPasteDataTransfer: function(a, b) {
					if(this.isCustomCopyCutSupported && a && a.data && a.data.$) {
						...
						this.copyCutData && c.id == this.copyCutData.id ? (c = this.copyCutData,
							c.$ = a.data.$.clipboardData) : this.copyCutData = c;
						return c
					}
			...
			CKEDITOR.plugins.clipboard.dataTransfer.prototype = {
				...
				getFile: function(a) {
					return this._.files.length ? this._.files[a] : this.$ && this.$.files && this.$.files.length ? this.$.files[a] : 0 === a ?
				...
				CKEDITOR.tools.createClass({
					...
					proto: {
						...
						onKeyDown: function(a, b) {
							...
							switch(e) {
								case "next":
									for(var l = this._.focusIndex, e = this.element.getElementsByTag("a"), f; f = e.getItem(++l);)
				...
				proto: {
					...
					showBlock: function(a, c, d, e, l, f) {
						...
						var n = this.element,
							...
							v = c.getDocumentPosition(u),
							u = q ? q.getDocumentPosition(u) : {
								x: 0,
								y: 0
							},
							t = "rtl" == this._.dir,
						...
						CKEDITOR.tools.setTimeout(function() {
							var a = CKEDITOR.tools.bind(function() {
								...
								t ? e < c && (p = g > c ? p + c : b.width > c ? p - a.left : p - a.right + b.width) : e < c && (p = g > c ? p - c : b.width >
									c ? p - a.right + b.width : p - a.left);
								c = a.top;
			...
			CKEDITOR.menu = CKEDITOR.tools.createClass({
				...
				proto: {
					...
					show: function(b, c, e, f) {
						...
						a(h);
						for(var u = k.elementPath(), u = ['\x3cdiv class\x3d"cke_menu' + (u && u.direction() != k.lang.dir ? " cke_mixed_dir_content" : "") + '" role\x3d"presentation"\x3e'], q = h.length, v = q && h[0].group, t = 0; t < q; t++) {
							var p = h[t];
		...
		function() {
			function a(a, b) {
				...
				g.onClick = l;
				var u = CKEDITOR.tools.addFunction(l),
				...
				a.on("selectionChange", function() {
					for(var b = [], f = g.list = [], e = [], m = g.filters, l = !0, B = a.elementPath().elements, r, z = B.length; z--;) {
						var E = B[z],
							y = 0;
						r = E.data("cke-display-name") ? E.data("cke-display-name") : E.data("cke-real-element-type") ? E.data("cke-real-element-type") : E.getName();
						...
						y || (f.unshift(E), e.unshift(r))
					}
					f = f.length;
					for(m = 0; m < f; m++) r = e[m], l = a.lang.elementspath.eleTitle.replace(/%1/, r), r = c.output({
						id: n + m,
						label: l,
						text: r,
						...
						for(var e = new CKEDITOR.dom.walker(d), k; k = e.next();)
							if(k.type == CKEDITOR.NODE_ELEMENT)
								if(!k.equals(f) && k.getDirection()) d.setStartAfter(k),
									...
									if(g)
										for(var n = "ltr" == a.data.dir ? ["_rtl", ""] : ["", "_rtl"], u = 0; u < g.length; u++) k.hasClass(g[u] + n[0]) && (k.removeClass(g[u] + n[0]), k.addClass(g[u] + n[1]));
		...
		function() {
			function a(a) {
				function c(e) {
					...
					if(!h || !l) return !1;
					for(var v = h, h = [], t = !1; !t;) v.equals(l) && (t = !0), h.push(v), v = v.getNext();
					if(1 > h.length) return !1;
					v = e.getParents(!0);
					for(l = 0; l < v.length; l++)
						if(v[l].getName && m[v[l].getName()]) {
							e = v[l];
							break
						}
					for(var v =
							d.isIndent ? 1 : -1, l = h[0], h = h[h.length - 1], t = CKEDITOR.plugins.list.listToArray(e, g), z = t[h.getCustomData("listarray_index")].indent, l = l.getCustomData("listarray_index"); l <= h.getCustomData("listarray_index"); l++)
						if(t[l].indent += v, 0 < v) {
							var u = t[l].parent;
							t[l].parent = new CKEDITOR.dom.element(u.getName(), u.getDocument())
						}
					for(l = h.getCustomData("listarray_index") + 1; l < t.length && t[l].indent > z; l++) t[l].indent += v;
					h = CKEDITOR.plugins.list.arrayToList(t, g, null, a.config.enterMode, e.getDirection());
					if(!d.isIndent) {
						var y;
						if((y = e.getParent()) && y.is("li"))
							for(var v = h.listNode.getChildren(), A = [], C, l = v.count() - 1; 0 <= l; l--)(C = v.getItem(l)) && C.is && C.is("li") && A.push(C)
					}
					h && h.listNode.replace(e);
					if(A && A.length)
						for(l = 0; l < A.length; l++) {
							for(C = e = A[l];
								(C = C.getNext()) && C.is && C.getName() in m;) CKEDITOR.env.needsNbspFiller && !e.getFirst(b) && e.append(q.document.createText(" ")), e.append(C);
							e.insertAfter(y)
						}
				...
				for(var d = this, g = this.database, m = this.context, l = a.getSelection(), l = (l && l.getRanges()).createIterator(),
					...
					if(!v) {
						var t = q.getEnclosedNode();
						t && t.type == CKEDITOR.NODE_ELEMENT && t.getName() in m && (q.setStartAt(t, CKEDITOR.POSITION_AFTER_START), q.setEndAt(t, CKEDITOR.POSITION_BEFORE_END), v = t)
					}
					v && q.startContainer.type == CKEDITOR.NODE_ELEMENT && q.startContainer.getName() in m && (t =
						new CKEDITOR.dom.walker(q), t.evaluator = e, q.startContainer = t.next());
					v && q.endContainer.type == CKEDITOR.NODE_ELEMENT && q.endContainer.getName() in m && (t = new CKEDITOR.dom.walker(q), t.evaluator = e, q.endContainer = t.previous());
					if(v) return c(v)
				}
				return 0
			}
		...
		function() {
			function a(a, b, c) {
				function d(c) {
					if(!(!(m = k[c ? "getFirst" : "getLast"]()) || m.is && m.isBlockBoundary() || !(l = b.root[c ? "getPrevious" : "getNext"](CKEDITOR.dom.walker.invisible(!0))) || l.is && l.isBlockBoundary({
							br: 1
			...
			function m(a, d, f) {
				...
				var h = d.createBookmark(),
					...
					u = q.contains(CKEDITOR.dtd.$listItem),
				...
				u && (e = l(u)) && (k.contains(u) ? (b(e, u.getParent(), u), e.remove()) : k.append(e));
				...
				listToArray: function(a, b, c, d, e) {
					...
					for(var g = 0, h = a.getChildCount(); g < h; g++) {
						...
						if("li" == k.$.nodeName.toLowerCase()) {
							...
							c.push(m);
							for(var l = 0, n = k.getChildCount(), q; l < n; l++) q = k.getChild(l), q.type == CKEDITOR.NODE_ELEMENT && f[q.getName()] ? CKEDITOR.plugins.list.listToArray(q, b, c, d + 1, m.grandparent) : m.contents.push(q)
						}
					}
					return c
				},
				arrayToList: function(a, b, c, d, e) {
					...
					for(var h, m = a[c].parent.getDocument(), l = new CKEDITOR.dom.documentFragment(m), n = null, q = c, u = Math.max(a[c].indent, 0), C = null, G, F, I = d == CKEDITOR.ENTER_P ? "p" : "div";;) {
						...
						if(H.indent == u) {
							...
							q++
						} else if(H.indent == Math.max(u, 0) + 1) H = a[q - 1].element.getDirection(1), q = CKEDITOR.plugins.list.arrayToList(a, null, q, d, H != G ? G : null), !C.getChildCount() && CKEDITOR.env.needsNbspFiller && 7 >= m.$.documentMode && C.append(m.createText(" ")), C.append(q.listNode), q = q.nextIndex;
						else if(-1 ==
							H.indent && !c && h) {
							...
							var n = h.getDirection(1) != G,
								K = H.element,
								D = K.getAttribute("class"),
								N = K.getAttribute("style"),
								R = C.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT && (d != CKEDITOR.ENTER_BR || n || N || D),
								P, T = H.contents.length,
								O;
							for(h = 0; h < T; h++)
								if(P = H.contents[h], k(P) && 1 < T) R ? O = P.clone(1, 1) : C.append(P.clone(1, 1));
			...
			e.prototype = {
				exec: function(b) {
					...
					for(var d = e.createBookmarks(!0), k = [], l = {}, h = h.createIterator(), n = 0;
						(m = h.getNextRange()) && ++n;) {
						var q = m.getBoundaryNodes(),
							y = q.startNode,
							A = q.endNode;
						...
						m = m.createIterator();
						for(m.forceBrBreak =
							this.state == CKEDITOR.TRISTATE_OFF; q = m.getNextParagraph();)
							if(!q.getCustomData("list_block")) {
								CKEDITOR.dom.element.setMarker(l, q, "list_block", 1);
								for(var C = b.elementPath(q), y = C.elements, A = 0, C = C.blockLimit, G, F = y.length - 1; 0 <= F && (G = y[F]); F--)
					...
					for(G = []; 0 < k.length;)
						if(y = k.shift(), this.state == CKEDITOR.TRISTATE_OFF)
							if(f[y.root.getName()]) {
								...
								C = [];
								for(q = 0; q < n.contents.length; q++) F = n.contents[q], (F = F.getAscendant("li", !0)) && !F.getCustomData("list_item_processed") && (C.push(F), CKEDITOR.dom.element.setMarker(y, F, "list_item_processed", !0));
						...
						(a.ui.addButton("NumberedList", {
							...
							if("wysiwyg" == a.mode && c in {
								...
								if(h && h.collapsed) {
									var q = 8 == c,
										u = a.editable(),
										E = new CKEDITOR.dom.walker(h.clone());
					...
					if(m = m || e(a)) {
						...
						if(v && t) {
							if(w && (w.is("li") || w.getParent().is("li"))) {
								...
								var r = !w.hasNext(),
									...
									y = B.getDirection(1) != v;
								a = a.enterMode != CKEDITOR.ENTER_BR || y || E || t;
								if(B.is("li")) u || r ? (u && r && m.remove(), w[r ? "insertAfter" : "insertBefore"](B)) :
									w.breakParent(B);
								else {
									if(a)
										if(p.block.is("li") ? (B = q.createElement(f == CKEDITOR.ENTER_P ? "p" : "div"), y && B.setAttribute("dir", v), E && B.setAttribute("style", E), t && B.setAttribute("class", t), w.moveChildren(B)) : B = p.block, u || r) B[u ? "insertBefore" : "insertAfter"](m);
										else w.breakParent(m), B.insertAfter(m);
									else if(w.appendBogus(!0), u || r)
										for(; q = w[u ? "getFirst" : "getLast"]();) q[u ? "insertBefore" : "insertAfter"](m);
									else
										for(w.breakParent(m); q = w.getLast();) q.insertAfter(m);
									w.remove()
								}
								x.selectBookmarks(z);
								return
						...
						if(v = m.splitBlock(x)) {
							...
							if(p || a) {
								...
								B ? u && !B.is("li") && B.renameNode(x) : z && z.is("li") ? B = z : (B = q.createElement(x), f && (r = f.getDirection()) && B.setAttribute("dir", r));
								if(q = v.elementPath)
									for(u = 0, x = q.elements.length; u < x; u++) {
										z = q.elements[u];
		...
		function() {
			...
			function m(a, f, g, l) {
				if(l && l.length)
					for(var u, q = l.length; q--;)
						if(u = l[q], "hbox" != u.type && "vbox" != u.type && "fieldset" != u.type || m(a, f, g, u.children), u.filebrowser)
				...
				if("function" != typeof b || !1 !== b.call(this._.filebrowserSe))
					if(!f || !1 !== f.call(this._.filebrowserSe, a, b))
						if("string" == typeof b && b && alert(b), a && (d = this._.filebrowserSe, c = d.getDialog(), d = d.filebrowser.target || null))
							if(d = d.split(":"), f = c.getContentElement(d[0],
			...
			function a(a) {
				var d =
					...
					l = function() {
						...
						var k, m, t, p, w, x, B, r = d.floatSpaceDockedOffsetX || 0,
							z = d.floatSpaceDockedOffsetY || 0,
							E = d.floatSpacePinnedOffsetX || 0,
						...
						return function(f) {
							if(m =
								...
								k ? (x + z <= p.top ? g("top") : x + z > w.height - p.bottom ? g("pin") : g("bottom"), f = w.width / 2, f = d.floatSpacePreferRight ? "right" : 0 < p.left && p.right < w.width && p.width > t.width ? "rtl" == d.contentsLangDirection ? "right" : "left" : f - p.left >
									p.right - f ? "left" : "right", t.width > w.width ? (f = "left", n = 0) : (n = "left" == f ? 0 < p.left ? p.left : 0 : p.right < w.width ? w.width - p.right : 0, n + t.width > w.width && (f = "left" == f ? "right" : "left", n = 0)), h.setStyle(f, b(("pin" == k ? E : r) + n + ("pin" == k ? 0 : "left" == f ? B : -B)))) : (k = "pin", g("pin"), l(f))
							}
						}
					}();
				if(m) {
					var f = new CKEDITOR.template('\x3cdiv id\x3d"cke_{name}" class\x3d"cke {id} cke_reset_all cke_chrome cke_editor_{name} cke_float cke_{langDir} ' + CKEDITOR.env.cssClass + '" dir\x3d"{langDir}" title\x3d"' + (CKEDITOR.env.gecko ? " " : "") + '" lang\x3d"{langCode}" role\x3d"application" style\x3d"{style}"' +
							(a.title ? ' aria-labelledby\x3d"cke_{name}_arialbl"' : " ") + "\x3e" + (a.title ? '\x3cspan id\x3d"cke_{name}_arialbl" class\x3d"cke_voice_label"\x3e{voiceLabel}\x3c/span\x3e' : " ") + '\x3cdiv class\x3d"cke_inner"\x3e\x3cdiv id\x3d"{topId}" class\x3d"cke_top" role\x3d"presentation"\x3e{content}\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e'),
				...
				CKEDITOR.tools.createClass({
					...
					proto: {
						...
						setValue: function(a, c) {
							...
							d && (a || c ? d.removeClass("cke_combo_inlinelabel") : (c = this.label, d.addClass("cke_combo_inlinelabel")), d.setText("undefined" !=
								typeof c ? c : a))
						},
						getValue: function() {
							return this._.value || ""
		...
		function() {
			...
			CKEDITOR.plugins.link = {
				...
				parseLinkAttributes: function(a, b) {
					...
					if(!E.type)
						if(r = c.match(h)) E.type = "anchor", E.anchor = {}, E.anchor.name = E.anchor.id = r[1];
						else if(r = c.match(m)) {
						z = c.match(l);
						c = c.match(f);
						E.type = "email";
					...
					if(b) {
						if(c = b.getAttribute("target")) E.target = {
							...
							for(E.target = {
									type: "popup",
									name: c[1]
								}; r = v.exec(c[2]);) "yes" != r[2] && "1" != r[2] || r[1] in {
								height: 1,
								width: 1,
								top: 1,
								left: 1
							} ? isFinite(r[2]) && (E.target[r[1]] = r[2]) : E.target[r[1]] = !0;
						var c = {},
							A;
						for(A in t)(r = b.getAttribute(A)) && (c[t[A]] = r);
				...
				getLinkAttributes: function(c, d) {
					...
					if(d.advanced) {
						for(var q in t)(f = d.advanced[t[q]]) && (g[q] = f);
		...
		function() {
			...
			function x(a) {
				...
				var g = Math.min(g, 0 | h.size.outerHeight / 2),
					k = [],
					l, q;
				if(m(c.y, h.size.top - 1, h.size.top + g)) q = !1;
				else if(m(c.y, h.size.bottom - g, h.size.bottom + 1)) q = !0;
				else return null;
				...
				if(t) {
					...
					if(n(t)) {
						if(u(t) || !v(a, t) || t.getParent().is(W)) return null;
			...
			CKEDITOR.plugins.add("magicline", {
				init: function(a) {
					var c = a.config,
						m = c.magicline_triggerOffset || 30,
						l = {
							editor: a,
						...
						m.is(H.$inline) || (l.inInlineMode && !q(m) && m.setStyles({
							...
							if("wysiwyg" == a.mode &&
								...
								r = setTimeout(function() {
									...
									r = l.trigger = null;
									E(l);
									v && !l.hiddenMode && a.focusManager.hasFocus && !l.line.mouseNear() && (l.element = da(l, !0)) && ((l.trigger = w(l) || x(l) || ea(l)) && !t(l, l.trigger.upper || l.trigger.lower) ? l.line.attach().place() : (l.trigger = null, l.line.detach()), v = !1)
								}, 30)
							}
							...
							l.hiddenMode = 0;
							l.mouseDown = 0
						}), a.addCommand("accessPreviousSpace", k(l)), a.addCommand("accessNextSpace", k(l, !0)), a.setKeystroke([
			...
			var da = function() {
				...
				ea = function() {
					function b(f) {
						...
						for(var e = Number.MAX_VALUE, l, q, p, t; g && !g.equals(h) && (q = h.getNext(f.isRelevant));) l = Math.abs(c(f, h, q) - f.mouse.y), l < e && (e = l, p = h, t = q), h = q, r(f, h);
						if(!p || !t || !m(f.mouse.y, p.size.top, t.size.bottom)) return null;
						k.upper = p;
						k.lower = t;
						return k.set(R, 8)
					}
						...
						a.addCommand("maximize", {
							...
							exec: function() {
								...
								if(this.state == CKEDITOR.TRISTATE_OFF) {
									...
									for(var p = [t, v], x = 0; x < p.length; x++) c(p[x], p[x].getCustomData("maximize_saved_styles")), p[x].removeCustomData("maximize_saved_styles");
								...
								this.toggleState();
								if(p =
									this.uiItems[0]) t = this.state == CKEDITOR.TRISTATE_OFF ? f.maximize.maximize : f.maximize.minimize, p = CKEDITOR.document.getById(p._.id), p.getChild(1).setHtml(t), p.setAttribute("title", t), p.setAttribute("href", 'javascript:void("' + t + '");');
					...
					b.on("paste", function(e) {
						...
						if(f && (c || /(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(f))) {
							...
							var h = a(b, d, function() {
								if(h) b.fire("paste", l);
								else if(!b.config.pasteFromWordPromptCleanup || c || confirm(b.lang.pastefromword.confirmCleanup)) l.dataValue = CKEDITOR.cleanWord(f,
									b);
								c = 0
							});
							h && e.cancel()
		...
		CKEDITOR.config.removeFormatAttributes = "class,style,lang,width,height,align,hspace,valign", CKEDITOR.plugins.add("resize", {
			...
			bindEvents: function(a) {
				...
				a.on("beforeCommandExec", function(c) {
					...
					} else if("bold" === c.data.name || "italic" === c.data.name || "underline" === c.data.name || "strike" === c.data.name || "subscript" === c.data.name || "superscript" === c.data.name || "enter" === c.data.name)
						if(d = a.scayt) d.removeMarkupInSelectionNode(), setTimeout(function() {
			...
			menuGenerator: function(a, e) {
				...
				for(var l in c)
					if(-1 != CKEDITOR.tools.indexOf(m, l.replace("scayt_", "")) || "all" == a.config.scayt_contextCommands) d[l] = CKEDITOR.TRISTATE_OFF, "function" !== typeof c[l].verification ||
			...
			function l(b) {
				if(b instanceof CKEDITOR.dom.selection) {
					...
					a: {
						...
						CKEDITOR.dom.element.clearAllMarkers(g);k = b[0].getParent();
			...
			function k(b, c, d) {
				...
				for(c = 0; c < f.length; c++) {
					...
					var K = g.getParent(),
						...
						C = Math.max(C, P - u + N);
					A = Math.max(A, K - n + R);
					d || (N = g, (R = N.getBogus()) && R.remove(), N.trim(), g.getChildren().count() && (K == F || !D || D.isBlockBoundary && D.isBlockBoundary({
						br: 1
					}) || (F = I.getLast(CKEDITOR.dom.walker.whitespaces(!0)), !F || F.is && F.is("br") || I.append("br")), g.moveChildren(I)), c ? g.remove() : g.setHtml(""));
					F = K
				}
				if(d) return A *
					C == H;
			...
			function g(b, c) {
				...
				if(1 < m) {
					...
					for(var m = Math.floor(m /
							2), f = k + n, e = new CKEDITOR.dom.element(e.$.rows[f]), g = h(g, f), u, f = d.clone(), k = 0; k < g.length; k++)
						if(u = g[k], u.parentNode == e.$ && k > l) {
							f.insertBefore(new CKEDITOR.dom.element(u));
							break
						} else u = null;
					u || e.append(f)
				} else
					for(m = n = 1, e = f.clone(), e.insertAfter(f), e.append(f = d.clone()), u = h(g, k), l = 0; l < u.length; l++) u[l].rowSpan++;
				f.appendBogus();
				d.$.rowSpan = n;
				f.$.rowSpan = m;
				...
				init: function(c) {
					...
					t("columnDelete", h({
						...
						exec: function(b) {
							...
							for(var e = CKEDITOR.tools.buildTableMap(b), g, h, k = [], l = 0, m = e.length; l < m; l++)
								for(var n = 0, p = e[l].length; n < p; n++) e[l][n] == c.$ && (g = n), e[l][n] == d.$ && (h = n);
							for(l = g; l <= h; l++)
								for(n = 0; n < e.length; n++) d = e[n], c = new CKEDITOR.dom.element(b.$.rows[n]), d = new CKEDITOR.dom.element(d[l]), d.$ && (1 == d.$.colSpan ? d.remove() : --d.$.colSpan, n += d.$.rowSpan - 1, c.$.cells.length || k.push(c));
				...
				function b() {
					for(var g = c(), h = CKEDITOR.tools.clone(a.config.toolbarGroups) ||
						...
						if("/" != m) {
							...
							var t, p = m.groups;
							if(p)
								for(var w = 0; w < p.length; w++) t = p[w], (t = g[t]) && f(m, t);
							(t = g[m.name]) && f(m, t)
						}
					}
					return h
						...
						if(c.data.space == d.config.toolbarLocation) {
							...
							for(var u = d.toolbox.toolbars, q = a(d), v = 0; v < q.length; v++) {
								var t, p = 0,
									...
									B;
								if(x)
									if(g && (k.push("\x3c/span\x3e"), m = g = 0), "/" === x) k.push('\x3cspan class\x3d"cke_toolbar_break"\x3e\x3c/span\x3e');
									else {
										B = x.items || x;
		...
		function() {
			...
			function e(a) {
				...
				} catch(h) {} else CKEDITOR.env.ie && 11 >
					CKEDITOR.env.version && a.config.disableObjectResizing && b(a)
			}
					...
					a.addMode("wysiwyg", function(b) {
						...
						if(n) {
							var g = CKEDITOR.tools.getNextId(),
								u = CKEDITOR.dom.element.createFromHtml('\x3cspan id\x3d"' + g + '" class\x3d"cke_voice_label"\x3e' + n + "\x3c/span\x3e");
							f.append(u, 1);
